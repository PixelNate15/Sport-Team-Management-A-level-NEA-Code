...
from Reusable_Code.view_result_information import View_Result_Details
...
from SQL import get_partner_feedback_for_one_fixture
import tkinter as tk


class Member_Expanded_Result_Card_Screen(tk.Frame):
    def __init__(self, parent, app, user_id, result):
        ...
        self.feedback = get_partner_feedback_for_one_fixture(result["fixture_id"], self.user_id)
        self.feedback_frame = Feedback_Cards(self, self.feedback)
        self.feedback_frame.pack()
        
        #Put back button onto the screen
        self.btn_back = Rounded_Button(self, text="Back", command=self.return_to_recent_results_screen, font=c.FONT_BUTTON, bg_colour=c.LIGHT_PRIMARY_ACCENT, hover_colour=c.LIGHT_ACCENT_HOVER, fg_colour="white", width=170, height=42, radius=16, parent_bg=c.LIGHT_BACKGROUND)
        self.btn_back.pack(pady=15)
        
        
    #Method to return to the recent_results_screen
    def return_to_recent_results_screen(self):
